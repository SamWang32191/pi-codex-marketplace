...
import type { BridgeState, ReadResult, ReadStatus } from './types.js';
...
async function rejectedAsStale(
  ...
  const appended = await appendReceipt(receipt, opts);
  if (!appended.success) {
...
async function appendConditionally(
  ...
  if (appended.isStale) {
    return rejectedAsStale(expected, {
      ...observed,
...
export async function repairBridgeState(
  ...
  try {
    ...
    return await withBridgeStateLock(opts, async (stateRead) => {
      try {
        ...
        if (!sameExpectation(expected, observed)) {
          return await rejectedAsStale(expected, observed, opts);
        }
        ...
        if (stateRead.status !== 'ok' && stateRead.status !== 'missing') {
          const finding = indeterminateFinding(statePath, stateRead);
          const receipt = createReceipt({
            kind: 'State Repair',
            operation: 'Repair State',
            trigger: 'repair state',
          ...
          const stale = await appendConditionally(receipt, expected, observed, opts);
          return stale ?? {
            success: false,
            status: 'persistence-indeterminate',
        ...
        if (committed.status === 'stale') {
          ...
          return await rejectedAsStale(expected, {
            ...observed,
