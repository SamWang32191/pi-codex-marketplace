...
describe('Git Marketplace Plugin Inspection & Installation lifecycle (#34)', () => {
  ...
  beforeEach(async () => {
    ...
    const preflight = await preflightGitRegistration('https://github.com/acme/marketplace.git',
  ...
  it('inspectMarketplaceEntries reads cached Git tree and returns available Plugin entries', async () => {
    ...
    const inspection = inspectMarketplaceEntries(registration, opts);
  ...
  it('completes Install and Enable lifecycle and enables disabled installation for Git Plugin', async () => {
    ...
    const preflight2 = await preflightPluginInstallation(registrationId, '/plugins/0', opts);
    ...
    // Disable plugin
    const disabled = await disablePluginInstallation(outcome.installation.id, opts);
    ...
    const enablePreflight = await preflightPluginEnable(outcome.installation.id, opts);
